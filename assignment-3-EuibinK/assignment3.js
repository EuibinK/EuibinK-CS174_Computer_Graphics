...
export class Assignment3 extends Scene {
    constructor() {
        ...
        this.materials = {
            ...
            sun: new Material(new defs.Phong_Shader(),
                {ambient: 1, color: color(0, 0, 0, 1)}),

            planet1: new Material(new defs.Phong_Shader(),
                {diffusivity: 1, color: hex_color("#808080")}),

            planet2_phong: new Material(new defs.Phong_Shader(),
                {diffusivity: 0.5, color: hex_color("#0D2D35"), specularity: 1}),

            planet2_gouraud: new Material(new Gouraud_Shader(),
                {diffusivity: 0.5, color: hex_color("#0D2D35"), specularity: 1}),

            planet3: new Material(new defs.Phong_Shader(),
                {diffusivity: 1, color: hex_color("#CB6649"), specularity: 1}),

            planet4: new Material(new defs.Phong_Shader(),
                {specularity: 0.8, color: hex_color("#00DBFF"), smoothness: 100}),

            moon: new Material(new defs.Phong_Shader(),
                {color: hex_color("#003300")})
    ...
    display(context, program_state) {
        ...
        // Sun
        const sun_r = 2 + Math.sin(0.2 * Math.PI * t);
        const sun_c = (1 + Math.sin(0.2 * Math.PI * t)) / 2;

        this.color = (x) => color(x, 0, 1 - x, 1);
        this.lights = (r) => [new Light(vec4(0, 0, 0, 1), color(1, 1, 1, 1), 10 ** r)];
        program_state.lights = this.lights(sun_r);

        let sun_transform = Mat4.identity();
        sun_transform = sun_transform.times(Mat4.scale(sun_r, sun_r, sun_r));
        this.shapes.sphere4.draw(context, program_state, sun_transform, this.materials.sun.override({color: this.color(sun_c)}));

        // Planet 1
        let period = t;
        let planet1_transform = Mat4.identity();
        planet1_transform = planet1_transform.times(Mat4.rotation(period, 0, 1, 0))
            .times(Mat4.translation(-5, 0, 0))
            .times(Mat4.rotation(period, 0, 1, 0));
        this.shapes.sphere2.draw(context, program_state, planet1_transform, this.materials.planet1);
        this.planet_1 = planet1_transform;

        // Planet 2
        let period2 = period * Math.sqrt(125 / 512);
        let planet2_transform = Mat4.identity();
        planet2_transform = planet2_transform.times(Mat4.rotation(period2, 0, 1, 0))
            .times(Mat4.translation(8, 0, 0))
            .times(Mat4.rotation(period2, 0, 1, 0));
        if (Math.round(t) % 2 == 0) {
        ...
        // Planet 3
        let period3 = period * Math.sqrt(125 / 1331);
        let planet3_transform = Mat4.identity();
        planet3_transform = planet3_transform.times(Mat4.rotation(period3, 0, 1, 0))
            .times(Mat4.translation(-11, 0, 0))
            .times(Mat4.rotation(Math.PI / 2, 1, 0, 0))
            .times(Mat4.rotation(2 * Math.sin(t) / 3, 1, 0, 1))
            .times(Mat4.rotation(period3, 0, 0, 1));

        this.shapes.sphere4.draw(context, program_state, planet3_transform, this.materials.planet3);
        this.shapes.torus.draw(context, program_state, planet3_transform.times(Mat4.scale(2, 2, 0.05)), this.materials.ring);
        this.planet_3 = planet3_transform;

        // Planet 4
        let period4 = period * Math.sqrt(125 / 2744);
        let planet4_transform = Mat4.identity();
        planet4_transform = planet4_transform.times(Mat4.rotation(period4, 0, 1, 0))
            .times(Mat4.translation(14, 0, 0))
            .times(Mat4.rotation(period3, 0, 1, 0));
        ...
        moon_transform = moon_transform.times(Mat4.rotation(period2, 2, 0, 0))
            .times(Mat4.translation(0, 2, 0))
            .times(Mat4.rotation(period2, 0, 1, 0));

        this.shapes.sphere1.draw(context, program_state, moon_transform, this.materials.moon);
...
class Ring_Shader extends Shader {
    ...
    fragment_glsl_code() {
        ...
        return this.shared_glsl_code() + `
        void main(){
            float m_scalar = sin(75.0 * distance(center, point_position));
            gl_FragColor = m_scalar * vec4( .865, .398, .285, 1);
        }`;
    }
